...
class Metrics:
    ...
    def average(metrics: Sequence[Metrics]) -> Metrics:
        ...
        metrics_means = {
            k: np.mean(
                [
                    d[k] for d in metrics_dicts if d[k] is not None
                ]  # handle case where cut discrepancy is undefined
            )
...
def compute_metrics(
    ...
    estimated: NumpyTensor["f"],
    reference: NumpyTensor["f"],
) -> Metrics:
    """ """
    metrics = {}
    segment_sizes = _compute_segment_sizes(estimated, reference)
    rm = (
        _compute_directional_hamming_distance(
            segment_sizes.reference, segment_sizes.intersect.T
        )
        / segment_sizes.total_faces
    )
    rf = (
        _compute_directional_hamming_distance(
            segment_sizes.estimated, segment_sizes.intersect
        )
        / segment_sizes.total_faces
    )
    ...
    metrics["gce"], metrics["lce"] = _compute_consistency_error(
        segment_sizes, estimated, reference
    )
    metrics["cut_discrepancy"] = compute_cut_discrepancy(mesh, estimated, reference)
    return Metrics(**metrics)


def _compute_segment_sizes(
    estimated: NumpyTensor["f"], reference: NumpyTensor["f"]
) -> SegmentSizes:
    """ """
...
def _compute_directional_hamming_distance(
    s2_sizes: NumpyTensor, intersect_sizes: NumpyTensor
) -> float:
    """ """
...
def _compute_consistency_error(
    sizes: SegmentSizes, estimated: NumpyTensor["f"], reference: NumpyTensor["f"]
) -> tuple[float, float]:
    """ """
...
def benchmark_dataset_princeton_one(
    ...
    if load_json:
        segmentation = seg_from_face2label(
            f"{path_segmentations}/{filename}/{filename}_face2label.json"
        )
    else:
...
def benchmark_dataset_princeton(
    ...
    chunks = [
        ...
        if extract_category(i) not in [14]  # , 4, 8, 13, 17]
    ]
    ...
    for cat in categories:
        ...
        chunks.extend(
            [
                (
                    cat_path_meshes,
                    cat_path_segmentations,
                    cat_path_segmentations_reference,
                    i,
                    cat,
                )
...
if __name__ == "__main__":
    ...
    #     path_segmentations ='/content/samesh/outputs/mesh_segmentation_output_princeton_shape_diameter_function',
    ...
    #     path_meshes='/home/gtangg12/data/samesh/MeshsegBenchmark-1.0/data/off',
    #     path_segmentations ='/content/samesh/outputs/mesh_segmentation_output_princeton',
    #     path_segmentations_reference='/home/gtangg12/data/samesh/MeshsegBenchmark-1.0/data/seg/Benchmark',
    #     load_json=True
    # )
    # print(metrics1)
    ...
    metrics2 = benchmark_dataset_princeton(
        path_meshes="/home/gtangg12/data/samesh/MeshsegBenchmark-1.0/data/off",
        path_segmentations="/content/samesh/outputs/mesh_segmentation_output_princeton_dynamic",
        path_segmentations_reference="/home/gtangg12/data/samesh/MeshsegBenchmark-1.0/data/seg/Benchmark",
        load_json=True,
    )
    print(metrics1)
    ...
    #     path_meshes='/content/sameshdata/coseg',
    #     path_segmentations='/content/samesh/outputs/mesh_segmentation_output_coseg_shape_diameter_function',
    #     path_segmentations_reference='/home/gtangg12/datacoseg',
    # )
    # metrics2 = benchmark_dataset_coseg(
    #     path_meshes='/content/sameshdata/coseg',
    #     path_segmentations='/content/samesh/outputs/mesh_segmentation_output_coseg',
    #     path_segmentations_reference='/home/gtangg12/datacoseg',
    # )
    # print(metrics1)
    # print(metrics2)
